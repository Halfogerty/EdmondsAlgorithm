...
def test_lift_path_blossom_is_endpoint():
    ...
    assert lift_path(test_augmenting_path, test_blossom, test_forest, test_graph) == ['G', 'H', 'D', 'C', 'B', 'A']
...
def test_find_augmenting_path():
    ...
    assert augmenting_path == expected_path or augmenting_path == expected_path[::-1]
...
def test_find_augmenting_path_recurse():
    test_graph = Graph(
        {'A': {'B'}, 'B': {'C'}, 'C': {'D'}, 'D': {'E'}, 'E': {'F'}, 'F': {'G', 'J'}, 'G': {'H'}, 'H': {'I'},
         'I': {'J'}})
    test_matching = Matching({Edge('B', 'C'), Edge('D', 'E'), Edge('F', 'G'), Edge('I', 'J')})
    print(find_augmenting_path(test_graph, test_matching))


def test_simpler():
    test_graph = Graph({'A': {'C', 'B'}, 'B': {'C'}, 'C': {'D'}, 'D': {'E', 'F'}, 'E': {'F'}})
    test_matching = Matching({Edge('B', 'C'), Edge('D', 'F')})
    print(find_augmenting_path(test_graph, test_matching))
